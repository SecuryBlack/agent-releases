...
# Uso: curl -fsSL https://install.securyblack.com/agent.sh | sudo bash
...
download_agent() {
    ...
    ASSET_NAME="${AGENT_NAME}-linux-${ARCH}"
    ...
    cat > "${CONFIG_DIR}/appsettings.json" <<EOF
{
  ...
  "UpdateCheckIntervalSeconds": 3600,
    ...
    "UpdateRepository": "${GITHUB_REPO}",
    "ConfigFilePath": "${CONFIG_DIR}/config.json",
    "IsRegistered": false,
...
show_post_install_info() {
    ...
    echo "   • Desinstalar: curl -fsSL https://install.securyblack.com/uninstall.sh | sudo bash"
    echo ""
